...
public class RocketMQTemplate extends AbstractMessageSendingTemplate<String> implements InitializingBean, DisposableBean {
    ...
    public SendResult syncSend(String destination, Message<?> message, long timeout) {
        ...
        try {
            ...
            org.apache.rocketmq.common.message.Message rocketMessage = RocketMQUtil.convertToRocketMessage(destination, message, objectMapper, charset);
    ...
    public SendResult syncSendOrderly(String destination, Message<?> message, String hashKey, long timeout) {
        ...
        try {
            long now = System.currentTimeMillis();
            org.apache.rocketmq.common.message.Message rocketMessage = RocketMQUtil.convertToRocketMessage(destination, message, objectMapper, charset);
            SendResult sendResult = defaultMQProducer.send(rocketMessage, messageQueueSelector, hashKey, timeout);
            long costTime = System.currentTimeMillis() - now;
            log.debug("send message cost: {} ms, msgId:{}", costTime, sendResult.getMsgId());
            return sendResult;
        } catch (Exception e) {
    ...
    public void asyncSend(String destination, Message<?> message, SendCallback sendCallback, long timeout) {
        ...
        try {
            org.apache.rocketmq.common.message.Message rocketMessage = RocketMQUtil.convertToRocketMessage(destination, message, objectMapper, charset);
            defaultMQProducer.send(rocketMessage, sendCallback, timeout);
        } catch (Exception e) {
    ...
    public void asyncSendOrderly(String destination, Message<?> message, String hashKey, SendCallback sendCallback,
        ...
        try {
            org.apache.rocketmq.common.message.Message rocketMessage = RocketMQUtil.convertToRocketMessage(destination, message, objectMapper, charset);
            defaultMQProducer.send(rocketMessage, messageQueueSelector, hashKey, sendCallback, timeout);
        } catch (Exception e) {
    ...
    public void sendOneWay(String destination, Message<?> message) {
        ...
        try {
            org.apache.rocketmq.common.message.Message rocketMessage = RocketMQUtil.convertToRocketMessage(destination, message, objectMapper, charset);
            defaultMQProducer.sendOneway(rocketMessage);
        } catch (Exception e) {
    ...
    public void sendOneWayOrderly(String destination, Message<?> message, String hashKey) {
        ...
        try {
            org.apache.rocketmq.common.message.Message rocketMessage = RocketMQUtil.convertToRocketMessage(destination, message, objectMapper, charset);
            defaultMQProducer.sendOneway(rocketMessage, messageQueueSelector, hashKey);
        } catch (Exception e) {
    ...
    private TransactionMQProducer stageMQProducer(String txProducerGroup) throws MessagingException {
        if (txProducerGroup == null) {
            txProducerGroup = RocketMQTransactionConstants.ROCKETMQ_TRANSACTION_GROUP;
        }
        TransactionMQProducer cachedProducer = txProducerCache.get(txProducerGroup);
        if (cachedProducer == null) {
            throw new MessagingException(String.format("Can not found MQProducer '%s' in cache! please define @RocketMQTransactionListener class or invoke createOrGetStartedTransactionMQProducer() to create it firstly", txProducerGroup));
        }
        return cachedProducer;
    }

    public TransactionSendResult sendMessageInTransaction(String txProducerGroup, String destination, final Message<?> message, final Object arg) throws MessagingException {
        org.apache.rocketmq.common.message.Message rocketMessage = RocketMQUtil.convertToRocketMessage(destination, message, getObjectMapper(), getCharset());

        TransactionMQProducer txProducer = this.stageMQProducer(txProducerGroup);
        try {
            return txProducer.sendMessageInTransaction(rocketMessage, arg);
        } catch (MQClientException e) {
            throw new MessagingException(e.getErrorMessage(), e);
        }
    }

    public TransactionSendResult sendMessageInTransaction(String txProducerGroup, String destination, final Object payload, final Object arg) throws MessagingException {
        return sendMessageInTransaction(txProducerGroup, destination, this.doConvert(payload, null, null), arg);
    }

    public TransactionSendResult sendMessageInTransaction(String destination, final Message<?> message, final Object arg) throws MessagingException {
        return sendMessageInTransaction(null, destination, message, arg);
    }

    public TransactionSendResult sendMessageInTransaction(String destination, final Object payload, final Object arg) throws MessagingException {
        return sendMessageInTransaction(null, destination, payload, arg);
    }

    public void shutdownTransactionMQProducer(String txProducerGroup) {
        if (txProducerCache.containsKey(txProducerGroup)) {
            DefaultMQProducer cachedProducer = txProducerCache.get(txProducerGroup);
            cachedProducer.shutdown();
            txProducerCache.remove(txProducerGroup);
        }
    }

    public void startTransactionMQProducer(String txProducerGroup, RocketMQLocalTransactionListener transactionListener, ExecutorService executorService) throws MessagingException {
        if (!txProducerCache.containsKey(txProducerGroup)) {
            TransactionMQProducer txProducer = initTransactionMQProducer(txProducerGroup, transactionListener, executorService);
            try {
                txProducer.start();
                txProducerCache.put(txProducerGroup, txProducer);
            } catch (MQClientException e) {
                throw new MessagingException(e.getErrorMessage(), e);
            }
        } else {
            log.info("get TransactionMQProducer '{}' from cache", txProducerGroup);
        }

    }

    private TransactionMQProducer initTransactionMQProducer(String txProducerGroup, RocketMQLocalTransactionListener transactionListener, ExecutorService executorService) {
        ...
        TransactionMQProducer txProducer = new TransactionMQProducer(txProducerGroup);
        txProducer.setTransactionListener(RocketMQUtil.convertToTransactionListener(transactionListener));
